...
\subsection{Demand file}

\paragraph*{\texttt{demand.txt} file is empty}
File is empty of entries as well as header data.

\paragraph*{Demand $x$ has non-positive id}
All entry ids must be positive.

\paragraph*{Duplicate demand id of $x$}
Each entry in the static OD file must have a separate id, and a duplicate id was found. This prevents loading demand properly.

\paragraph*{Demand $x$ type of $y$ not recognized}
The traveler or vehicle type for traveler $x$ is not recognized (see Section \ref{sec:staticod}).
...
\paragraph*{Demand $x$ has departure time of $y$}
The departure time is negative.

\paragraph*{Demand $x$ has value-of-time of $y$}
The value-of-time is negative.
